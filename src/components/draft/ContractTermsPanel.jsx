...
export default function ContractTermsPanel({ seasons, onSeasonsChange, annualWage, onAnnualWageChange, suggestedWage, wageCustomized, onResetWage, paymentLabel = 'paid at next season' }) {
    ...
    <div className="space-y-3">
      <div>
        ...
        <Select value={String(seasons)} onChange={event => onSeasonsChange(Number(event.target.value))} reserveErrorSpace={false}>
          {Array.from({ length: 10 }, (_, index) => index + 1).map(years => <option key={years} value={years}>{years} season{years > 1 ? 's' : ''}</option>)}
        </Select>
      </div>
      <div>
