...
export default function ContractTermsPanel({ seasons, onSeasonsChange, annualWage, onAnnualWageChange, suggestedWage, wageCustomized, onResetWage, paymentLabel = 'paid at next season', lengthLabel = 'Contract Length', maxSeasons = 10, totalLabel }) {
  const wageInputId = useId()
  const changeBy = millions => onAnnualWageChange(Math.max(0, annualWage + millions * 1_000_000))
  return (
    <div className="space-y-3">
      <div>
        <label className="mb-1 block text-xs font-heading font-bold uppercase tracking-wider text-gray-500">{lengthLabel}</label>
        <Select value={String(seasons)} onChange={event => onSeasonsChange(Number(event.target.value))} reserveErrorSpace={false}>
          {Array.from({ length: maxSeasons }, (_, index) => index + 1).map(years => <option key={years} value={years}>{lengthLabel === 'Extend By' ? '+' : ''}{years} season{years > 1 ? 's' : ''}</option>)}
        </Select>
      </div>
      <div>
        <div className="mb-1 flex items-center justify-between gap-2"><label htmlFor={wageInputId} className="text-xs text-gray-500">Wage / season · {paymentLabel}</label>{wageCustomized && <button type="button" onClick={onResetWage} className="cursor-pointer text-xs font-medium text-[#FD5461] hover:underline">Reset ${formatCurrency(suggestedWage)}</button>}</div>
        ...
        <div className="mt-2 flex items-center justify-between text-xs"><span className="text-gray-400">{totalLabel || `${seasons} season${seasons > 1 ? 's' : ''} total`}</span><span className="font-normal text-[#0A1318]">${formatCurrency(annualWage * seasons)}</span></div>
      </div>
    </div>
  )
}
