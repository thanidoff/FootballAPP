...
import { ArrowLeftRight, CalendarDays, ChartNoAxesColumn, Play, Search, Trophy } from 'lucide-react'
...
function Empty({ children }) {
  return <div className="flex min-h-40 items-center justify-center px-6 text-center text-sm text-gray-400">{children}</div>
}
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[minmax(460px,0.85fr)_minmax(0,1.15fr)]">
      <div className="space-y-6">
        <div>
          <LeagueStandingsTable standings={hasLeague ? standings : []} championId={activeSeason?.champion} onFullTable={() => navigate(`/draft/${saveId}/matches`)} onTeamClick={row => navigate(`/draft/${saveId}/squads?team=${row.club_id}`)} emptyContent={<div className="flex min-h-40 flex-col items-center justify-center px-6 text-center"><p className="text-sm text-gray-400">Create a league to see the standings.</p><button onClick={() => navigate(`/draft/${saveId}/matches`)} className="mt-4 rounded-xl border-2 border-[#FD5461] bg-white px-5 py-2 font-heading text-[10px] font-black uppercase tracking-wider text-[#FD5461] transition-colors hover:bg-red-50">Start League</button></div>} />
        </div>
          ...
          {upcoming.length ? (
                ...
                return (
                  <div key={`${match.week}-${index}`} className={`w-full px-5 py-3 text-left transition-[opacity,background-color] ${match.week > (saveData.currentWeek || 1) ? 'bg-gray-50/70 opacity-55' : 'hover:bg-red-50/40'}`}>
                    ...
                    <div className="grid grid-cols-[1fr_112px_1fr] items-center gap-2">
                      <span className="flex min-w-0 items-center gap-2"><Badge team={home} size="h-7 w-7" /><span className="truncate text-xs font-bold">{home?.club_name}</span></span>
                      {activeSeason?.status === 'active' && match.week === (saveData.currentWeek || 1)
                        ? <Button size="sm" variant="secondary" onClick={() => playMatch(match)} className="w-full whitespace-nowrap px-2"><Play size={13} fill="currentColor" />Play match</Button>
                        : <span className="text-center font-heading text-sm font-semibold text-[#0A1318]">VS</span>}
                      <span className="flex min-w-0 items-center justify-end gap-2"><span className="truncate text-right text-xs font-bold">{away?.club_name}</span><Badge team={away} size="h-7 w-7" /></span>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="flex min-h-40 flex-col items-center justify-center px-6 text-center">
              ...
              <button
                ...
                className="mt-4 rounded-xl bg-[#FD5461] px-5 py-2.5 font-heading text-[10px] font-black uppercase tracking-wider text-white shadow-sm shadow-red-500/20 transition-colors hover:bg-red-500"
              >
        ...
        <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          {recentResults.length ? <div className="divide-y divide-gray-50">{recentResults.map((match, index) => { const home = teamById(match.home), away = teamById(match.away); return <button key={`${match.week}-${match.matchIndex}-${index}`} onClick={() => navigate(`/draft/${saveId}/matches`)} className="w-full cursor-pointer px-5 py-3 text-left transition-colors hover:bg-slate-50"><div className="mb-2 text-xs font-medium text-gray-500">Week {match.week}</div><div className="grid grid-cols-[1fr_88px_1fr] items-center gap-2"><span className="flex min-w-0 items-center gap-2"><Badge team={home} size="h-7 w-7" /><span className="truncate text-xs font-bold">{home?.club_name}</span></span><ResultScore homeScore={match.homeScore} awayScore={match.awayScore} compact /><span className="flex min-w-0 items-center justify-end gap-2"><span className="truncate text-right text-xs font-bold">{away?.club_name}</span><Badge team={away} size="h-7 w-7" /></span></div></button> })}</div> : <Empty>No match results yet.</Empty>}
        </section>

        <section className="hidden">
          <header className="border-b border-gray-100 px-5 py-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2"><Trophy size={18} className="text-[#FD5461]" /><h2 className="font-heading text-base font-black uppercase tracking-wide text-[#0A1318]">Player Leaders</h2></div>
              <Select value={statScope} onChange={event => setStatScope(event.target.value)} reserveErrorSpace={false} className="min-h-10 rounded-xl py-1.5 text-sm">
                <option value="league">This league</option><option value="career">All seasons</option>
              </Select>
            </div>
            <div className="mt-4 flex gap-1 overflow-x-auto">
              {STAT_FILTERS.map(filter => <button key={filter.key} onClick={() => setStatFilter(filter.key)} className={`min-h-9 cursor-pointer whitespace-nowrap rounded-full px-4 text-xs font-medium transition-colors ${statFilter === filter.key ? 'bg-[#FD5461] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-900'}`}>{filter.label}</button>)}
            </div>
          </header>
          {leaders.length ? (
            <div className="divide-y divide-gray-50">{leaders.map(({ player, value }, index) => <div key={player.id} className="flex items-center gap-3 px-5 py-3"><span className="w-4 text-xs font-black text-gray-300">{index + 1}</span><span className="h-8 w-8 overflow-hidden rounded-full bg-gray-100">{player.photo_url && <img src={player.photo_url} alt="" className="h-full w-full object-cover" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-[#0A1318]">{player.name}</span><span className="block truncate text-[10px] text-gray-400">{player.team?.club_name || 'Free Agent'}</span></span><span className="font-heading text-xl font-black text-[#FD5461]">{value}</span></div>)}</div>
          ) : <Empty>No {STAT_FILTERS.find(filter => filter.key === statFilter)?.label.toLowerCase()} recorded yet.</Empty>}
        </section>
      </div>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <header className="border-b border-gray-100 px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <h2 className="font-heading text-lg font-black uppercase tracking-wide text-[#0A1318]">{featureView === 'transfers' ? 'Season Transfers' : 'Player Leaders'}</h2>
            <SegmentedControl items={[{ id: 'transfers', label: 'Transfers', icon: ArrowLeftRight }, { id: 'leaders', label: 'Player Leaders', icon: ChartNoAxesColumn }]} value={featureView} onChange={setFeatureView} ariaLabel="Dashboard feature" />
          </div>
          {featureView === 'leaders' && (
            <div className="mt-4 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex gap-1 overflow-x-auto hide-scrollbar">{STAT_FILTERS.map(filter => <button key={filter.key} onClick={() => setStatFilter(filter.key)} className={`min-h-9 cursor-pointer whitespace-nowrap rounded-full px-4 text-xs font-medium transition-colors ${statFilter === filter.key ? 'bg-[#FD5461] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-900'}`}>{filter.label}</button>)}</div><div className="w-40"><Select value={statScope} onChange={event => setStatScope(event.target.value)} reserveErrorSpace={false} className="min-h-10 rounded-xl py-1.5 text-sm"><option value="league">This league</option><option value="career">All seasons</option></Select></div></div>
        ...
        {featureView === 'transfers' ? (transfers.length ? (
          <div className="w-full overflow-hidden"><div className="grid grid-cols-[minmax(100px,1.5fr)_minmax(72px,0.8fr)_minmax(72px,0.8fr)_76px] gap-2 bg-gray-50 px-5 py-3 text-[9px] font-heading font-black uppercase tracking-widest text-gray-400"><span>Player</span><span>From</span><span>To</span><span className="text-right">Fee</span></div><div className="divide-y divide-gray-100">{transfers.map((item, index) => <div key={item.id || index} className="grid grid-cols-[minmax(100px,1.5fr)_minmax(72px,0.8fr)_minmax(72px,0.8fr)_76px] items-center gap-2 px-5 py-3 transition-colors hover:bg-red-50/30"><span className="truncate text-sm font-semibold text-[#0A1318]" title={item.playerName}>{item.playerName}</span><TransferClub team={teamById(item.fromClubId)} name={item.fromName} /><TransferClub team={teamById(item.toClubId)} name={item.toName} /><span className="text-right text-sm font-semibold tabular-nums text-[#FD5461]">{money(item.fee)}</span></div>)}</div></div>
        ) : <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center"><p className="text-sm text-gray-400">No transfers have been completed this season.</p><button onClick={() => navigate(`/draft/${saveId}/transfers`)} className="mt-4 rounded-xl bg-[#FD5461] px-5 py-2.5 font-heading text-[10px] font-black uppercase tracking-wider text-white shadow-sm shadow-red-500/20 transition-colors hover:bg-red-500">Open Market</button></div>) : (leaders.length ? (
          <div className="divide-y divide-gray-50">{leaders.map(({ player, value }, index) => <div key={player.id} className="grid grid-cols-[28px_40px_1fr_auto] items-center gap-3 px-5 py-3 transition-colors hover:bg-red-50/30"><span className={`text-center font-heading text-sm font-black ${index < 3 ? 'text-[#FD5461]' : 'text-gray-300'}`}>{index + 1}</span><span className="h-10 w-10 overflow-hidden rounded-full bg-gray-100">{player.photo_url && <img src={player.photo_url} alt="" className="h-full w-full object-cover" />}</span><span className="min-w-0"><span className="block truncate text-sm font-bold text-[#0A1318]">{player.name}</span><span className="block truncate text-xs text-gray-400">{player.team?.club_name || 'Free Agent'}</span></span><span className="font-heading text-2xl font-black text-[#FD5461]">{value}</span></div>)}</div>
        ) : <div className="flex min-h-[420px] flex-col items-center justify-center px-6 text-center"><p className="text-sm text-gray-400">No {STAT_FILTERS.find(filter => filter.key === statFilter)?.label.toLowerCase()} recorded yet.</p><button onClick={() => navigate(`/draft/${saveId}/matches`)} className="mt-4 rounded-xl bg-[#FD5461] px-5 py-2.5 font-heading text-[10px] font-black uppercase tracking-wider text-white shadow-sm shadow-red-500/20 transition-colors hover:bg-red-500">Open League</button></div>)}
      </section>
    </div>
  )
}
